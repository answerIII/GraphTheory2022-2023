...
#Вывести количество вершин и разбить по долям, сохранив в виде списков смежности
    ...
    if dvudol:
        print("Плотность графа: " + str(M / (len(RightVertex) * len(LeftVertex))))
    else:
    ...
    visited = set([])
    for index, row in data.iterrows():
        if ((row['in'], row['out']) in visited) or ((row['out'], row['in']) in visited):
            continue
        ...
        visited.add((row['in'], row['out']))

    M_reverse = 1 / (M)
    r = (multiply_m - M_reverse * ((0.5 * summary_m) ** 2)) / (
                0.5 * square_summary_m - M_reverse * ((0.5 * summary_m) ** 2))
