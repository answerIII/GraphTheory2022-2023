...
class Properties_calculator(Calculator):
    ...
    def calc(self, file):
        ...
        M = self.calculate_edge_count(AllVertex)
        print('Количество ребер в графе: ' + str(M))
        sys.setrecursionlimit(1000000)
        density = self.calculate_density(M, LeftVertex, RightVertex, countVertex) #вычисление плотности графа
        ...
        components = self.find_weak_components(AllVertex, countVertex)
        print('Количество компонент слабой связности: ' + str(len(components)))
        print('Доля вершин в максимальной по мощности компоненте слабой связности: ' + str(self.calculate_largest_component_ratio(components, countVertex)))
        ...
        if len(max_component) > 750: # Если компонент содержит более 750 вершин, создаем случайный подграф и подграф методом "снежный ком"
            ...
            snow_component = self.bfs_for_snow_subgraph([random.choice(max_component) for _ in range(3)], AllVertex)
        
            print('Метрики расстояний на случайном подграфе: ')
            random_diameter, random_radius, random_percentile = self.calculate_graph_metrics(random_component, AllVertex)
            ...
            snow_diameter, snow_radius, snow_percentile = self.calculate_graph_metrics(snow_component, AllVertex)
            ...
            diameter, radius, percentile = self.calculate_graph_metrics(max_component, AllVertex)
        ...
        average_clustering_coefficient = self.calculate_average_clustering_coefficient(AllVertex, countVertex)
        assortativity_coefficient = self.calculate_assortativity_coefficient(data, AllVertex, countVertex)
    ...
    def build_vertex_adjacency(self, data, is_dvudol): #Вычисление долей вершин, по спискам смежности
        AllVertex = {}
    ...
    def calculate_density(self, M, LeftVertex, RightVertex, countVertex): #Вычисление плотности графа
    ...
    def depth_first_search(self, AllVertex, vertex, visited, component): #DFS
        visited.add(vertex)
        ...
        for neighbor in neighbors:
            if neighbor not in visited:
                self.depth_first_search(AllVertex, neighbor, visited, component)

    def find_weak_components(self, AllVertex, countVertex): #Вычисление компонент слабой связности
        visited = set()
        components = []
        ...
        for vertex in AllVertex:
            ...
            if vertex not in visited:
                component = []
                self.depth_first_search(AllVertex, vertex, visited, component)
                components.append(component)
        
        return components

    def calculate_largest_component_ratio(self, components, countVertex): #Доля вершин в максимальной по мощности компоненте слабой связности
    ...
    def calculate_average_clustering_coefficient(self, AllVertex, countVertex):
    ...
    def calculate_assortativity_coefficient(self, data, AllVertex, countVertex):
        multiply_m = 0
        summary_m = 0
        square_summary_m = 0
        cube_summary_m = 0
